...
$nreserva= mysql_query("SELECT MAX(id_usuario) FROM $tabla41");
...
if ($id12=='GUARDAR'){
	...
	$id=$clave+1;
		...
		$sentencia="INSERT INTO $tabla41 ($campos) VALUES ($datos) ";
		...
		$inserta=mysql_query($sentencia,$conexion);
			...
			$sentencia_part="INSERT INTO $tabla42 ($campos_part) VALUES ($datos_part) ";
		...
		$sentencia_fecha="INSERT INTO $tabla43 ($campos_fecha) VALUES ($datos_fecha) ";
...
if ($actualizar=='ACTUALIZAR'){
	...
	/*------ ACTUALIZAR USUARIO $tabla41 ----*/
	$fecha=date("d-m-Y");
	$dia=date("j");
	$mes=date("n");
	$datos="fecha='$fecha',dia='$dia',mes='$mes'";
	mysql_query("UPDATE $base . `$tabla41` SET $datos WHERE id_usuario=$id_usuario");
	
	/*------ ACTUALIZAR PARTIDOS $tabla42 ----*/
	$partidos='0';
	...
	mysql_query("UPDATE $base . `$tabla42` SET $datos_partidos WHERE id_usuario=$id_usuario");
		
	/*------ ACTUALIZAR FECHA $tabla43 ----*/
	$fecha='0';
	$datos_fecha="b1='$fecha',b2='$fecha',b3='$fecha',b4='$fecha',b5='$fecha',b6='$fecha',b7='$fecha',b8='$fecha',b9='$fecha',b10='$fecha'";
	mysql_query("UPDATE $base . `$tabla43` SET $datos_fecha WHERE id_usuario=$id_usuario") or die(mysql_error());
	mysql_close();
